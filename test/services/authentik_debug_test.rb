...
class AuthentikDebugTest < ActiveSupport::TestCase
  ...
  test 'log_authorize_uri parses claims JSON from redirect query' do
    ENV['AUTHENTIK_DEBUG'] = 'true'
    claims = { userinfo: { is_admin: nil, has_slack: nil, slack: nil } }.to_json
    uri = "https://authentik.example.com/application/o/authorize/?client_id=abc&claims=#{CGI.escape(claims)}"
    ...
    assert_match(/"has_slack": null/, logs)
    assert_match(/"slack": null/, logs)
  end
